...
Reads CHANGELOG.md and writes the body of the requested version's section to stdout, without
the heading itself. Used by the release workflow::

    python scripts/extract_changelog.py 1.0.0

Exits non-zero if the version has no section, so a release cannot quietly publish with an
empty body.
"""
...
def main() -> int:
    ...
    if body is None:
        print(
            f"CHANGELOG.md has no section for version {version}. Add one before releasing.",
            file=sys.stderr,
        )
        return 1
    if not body:
