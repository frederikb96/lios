...
taken directly from the clipboard types the user observed on his own laptop after a Nautilus
Ctrl+C (the design) and from the priority table settled in the clipboard investigation
(the design, the clipboard investigation section 6/8).
"""
...
def test_nautilus_file_copy_picks_file_transfer_not_the_path_string() -> None:
    """The exact five types the user's laptop offered after Ctrl+C on a file in Nautilus."""
    available = [
