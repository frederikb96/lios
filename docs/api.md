...
- **`POST /api/items`** -- store a sealed item. Body is the raw sealed blob (`Content-Type: application/octet-stream`), *not* JSON -- a base64-encoded body would cost 33% more for an image. `Content-Length` is required; a body over `items.max_size_bytes` (see `relay/config/config.yaml`) is rejected with `413` before it is fully read. Everything else about the request that isn't the blob rides as a header, named in `lios_protocol.headers`:
  ...
  - `X-Target-Device-Id` (optional) -- narrows delivery to one device; omitted, the item broadcasts to every other currently-paired device. `404` if it names a device that does not exist.
